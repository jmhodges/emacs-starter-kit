...
;;; Slight modifications of http://www.emacswiki.org/emacs/ErcPageMe
...
(defun my-erc-page-popup-notification (nick msg)
  (when window-system
    ...
    (let ((default-directory "~/"))
      (growl (format my-erc-page-message nick) msg))))
...
(defun my-erc-page-me (match-type full-nick message)
  "Notify the current user when someone sends a message that
matches a regexp in `erc-keywords'."
  (interactive)
  (when (and (eq match-type 'keyword)
             ;; I don't want to see anything from the erc server
             (null (string-match "\\`\\([sS]erver\\|localhost\\)" full-nick))
             ;; or bots
...
(defun my-erc-page-me-PRIVMSG (proc parsed)
  (let ((nick (car (erc-parse-user (erc-response.sender parsed))))
    ...
    (when (and (erc-current-nick-p target)
      ...
      (my-erc-page-popup-notification nick "(in a private message)")
      nil)
    ))
